 ...
 */
final class RMQRST_Admin_Suggestions {
	...
	public static function instance() {
		...
		if (!isset(self::$instance))
			self::$instance = new RMQRST_Admin_Suggestions;
	...
	*/
	private function __construct() {
		...
		$timestamp = (int) get_option( 'rmqrst_dismissed_on' );

		if ( empty( $timestamp ) || ( time() - $timestamp ) > (90 * 86400) ) {

			// Check AJAX submit
			if (defined('DOING_AJAX') && DOING_AJAX) {
				add_action( 'wp_ajax_rmqrst_dismiss', array(&$this, 'dismiss'));
...
<script type="text/javascript">jQuery(function($) {	$(document).on('click', '.rmqrst-dismiss .notice-dismiss', function() { $.post(ajaxurl, {'action':'rmqrst_dismiss','nonce':$(this).parent().attr('data-nonce')}); }); });</script>
	<?php }
	...
	public function dismiss() {
		if (!empty($_POST['nonce']) && wp_verify_nonce($_POST['nonce'], 'rmqrst-dismiss')) {
			update_option('rmqrst_dismissed_on', time(), true);
		}
	}
	...
	public function admin_notices() {
		...
		?><div class="rmqrst-dismiss notice notice-success is-dismissible" data-nonce="<?php echo wp_create_nonce('rmqrst-dismiss'); ?>">

			<p>Remove Query Strings recommends the following free plugins:</p>
